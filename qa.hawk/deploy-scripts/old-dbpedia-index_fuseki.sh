...
if [ ! -d "apache-jena-3.4.0" ];
then
	wget http://archive.apache.org/dist/jena/binaries/apache-jena-3.4.0.zip
	unzip apache-jena-3.4.0.zip
	rm apache-jena-3.4.0.zip
fi

if [ ! -d "apache-jena-fuseki-3.4.0" ];
then
	wget http://archive.apache.org/dist/jena/binaries/apache-jena-fuseki-3.4.0.zip
	unzip apache-jena-fuseki-3.4.0.zip
	rm apache-jena-fuseki-3.4.0.zip
fi

# DBpedia server
echo "dbpedia_3.9.owl"
if [ ! -f "dbpedia_3.9.owl" ];
then
  wget http://downloads.dbpedia.org/3.9/dbpedia_3.9.owl
fi
if [ ! -f "disambiguations_en.ttl" ];
then
  wget http://downloads.dbpedia.org/3.9/en/disambiguations_en.ttl.bz2
  bunzip2 disambiguations_en.ttl.bz2
fi
if [ ! -f "instance_types_en.ttl" ];
then
  wget http://downloads.dbpedia.org/3.9/en/instance_types_en.ttl.bz2
  bunzip2 instance_types_en.ttl.bz2
fi
if [ ! -f "long_abstracts_en.ttl" ];
then
  wget http://downloads.dbpedia.org/3.9/en/long_abstracts_en.ttl.bz2 
...
then
  wget http://downloads.dbpedia.org/3.9/en/mappingbased_literals_en.ttl.bz2
  bunzip2 mappingbased_literals_en.ttl.bz2
fi
if [ ! -f "mappingbased_objects_en.ttl" ];
then
  wget http://downloads.dbpedia.org/3.9/en/mappingbased_objects_en.ttl.bz2
  bunzip2 mappingbased_objects_en.ttl.bz2
fi
if [ ! -f "persondata_en.ttl" ];
then
  wget http://downloads.dbpedia.org/3.9/en/persondata_en.ttl.bz2
  bunzip2 persondata_en.ttl.bz2
fi
if [ ! -f "specific_mappingbased_properties_en.ttl" ];
then
  wget http://downloads.dbpedia.org/3.9/en/specific_mappingbased_properties_en.ttl.bz2
  bunzip2 specific_mappingbased_properties_en.ttl.bz2
fi
if [ ! -f "transitive_redirects_en.ttl" ];
then
  wget http://downloads.dbpedia.org/3.9/en/transitive_redirects_en.ttl.bz2
  bunzip2 transitive_redirects_en.ttl.bz2
fi
if [ ! -f "instance_types_transitive_en.ttl" ];
then
  wget http://downloads.dbpedia.org/3.9/en/instance_types_transitive_en.ttl.bz2
...
export JENA_HOME=apache-jena-3.4.0
export FUSEKI_HOME=apache-jena-fuseki-3.4.0

APACHE_JENA_BIN=./apache-jena-3.4.0/bin
JENA_FUSEKI_JAR=apache-jena-fuseki-3.4.0/fuseki-server.jar

if [ ! -d "data/hawk_data_3.9_dbpediatbd" ];
then
	mkdir data
	cd data
        mkdir hawk_data_3.9_dbpediatbd/
        cd ..
	$APACHE_JENA_BIN/tdbloader2 --loc=data/hawk_data_3.9_dbpediatbd dbpedia_3.9.owl disambiguations_en.ttl instance_types_en.ttl long_abstracts_en.ttl labels_en.ttl mappingbased_objects_en.ttl mappingbased_literals_en.ttl persondata_en.ttl specific_mappingbased_properties_en.ttl en_surface_forms.ttl pagerank_en_2016-04.ttl instance_types_en.ttl instance_types_transitive_en.ttl
fi

if [ ! -d "data/hawk_data_3.9_dbpedialucene" ];
then
	cd data
	mkdir hawk_data_3.9_dbpedialucene
        cd ..
...
#rm *.bz2
#rm *.sorted
#rm -r apache-jena-3.4.0*
#rm -r apache-jena-fuseki-3.4.0		
#rm dbpedia_2016-10.owl* 
