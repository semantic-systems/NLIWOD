...
# DBpedia server
echo "dbpedia_2016-10.owl"
if [ ! -f "dbpedia_2016-10.owl" ];
then
  wget http://downloads.dbpedia.org/2016-10/dbpedia_2016-10.owl
fi
...
if [ ! -d "data/hawk_data_10-2016_dbpediatbd" ];
then
	mkdir data
	cd data
        mkdir hawk_data_10-2016_dbpediatbd/
        cd ..
	$APACHE_JENA_BIN/tdbloader2 --loc=data/hawk_data_10-2016_dbpediatbd dbpedia_2016-10.owl disambiguations_en.ttl instance_types_en.ttl long_abstracts_en.ttl labels_en.ttl mappingbased_objects_en.ttl mappingbased_literals_en.ttl persondata_en.ttl specific_mappingbased_properties_en.ttl en_surface_forms.ttl pagerank_en_2016-04.ttl instance_types_en.ttl instance_types_transitive_en.ttl
fi

if [ ! -d "data/hawk_data_10-2016_dbpedialucene" ];
then
	cd data
