...
public class SentenceToSequenceStanfordTest extends ANounCombinatorTest {
	StanfordNLPConnector connector = new StanfordNLPConnector();
	NounCombinationChain chain = new NounCombinationChain(NounCombiners.HawkRules);
	...
	public void test1() {
		...
		chain.runChain(q);
		Assert.assertTrue(containsExactly(q.getLanguageToNounPhrases().get("en"), "vice president", "atomic weapons", "World War II"));
	...
	public void test2() {
		...
		Assert.assertTrue(containsExactly(q.getLanguageToNounPhrases().get("en"), "Australian metalcore bands"));
	}
